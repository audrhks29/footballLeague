...
const NextMatch = memo((props: Props) => {
  ...
  } = teamData.competitors.find((item: { homeAway: string }) => item.homeAway === "home")
  const awayTeamData = teamData.competitors.find((item: { homeAway: string }) => item.homeAway === "away")
  ...
  const [{ data: fetchedHomeTeamData }, { data: fetchedAwayTeamData }]
    = useSuspenseQueries({
      queries: [
        {
          queryKey: ['fetchedHomeTeamData', props.fetchUrl],
          queryFn: () => fetchTeamData(homeTeamData.team.$ref),
        },
        {
          queryKey: ['fetchedAwayTeamData', props.fetchUrl],
          queryFn: () => fetchTeamData(awayTeamData.team.$ref),
        }
      ]
    });
  // console.log(teamData);
  const homeTeamLogo = fetchedHomeTeamData.logos[1] ? fetchedHomeTeamData.logos[1] : fetchedHomeTeamData.logos
  const awayTeamLogo = fetchedAwayTeamData.logos[1] ? fetchedAwayTeamData.logos[1] : fetchedHomeTeamData.logos
