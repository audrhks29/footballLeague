...
const OutfieldPlayer = memo(({ item, index }: Props) => {
  ...
  if (item.position.name !== "Goalkeeper" && item.statistics) {
    ...
    const totalGoals = item.statistics.splits.categories.find(category => category.name === 'offensive').stats.find(stat => stat.name === 'totalGoals');
    ...
    const totalShots = item.statistics.splits.categories.find(category => category.name === 'offensive').stats.find(stat => stat.name === 'totalShots');
    const shotsOnTarget = item.statistics.splits.categories.find(category => category.name === 'offensive').stats.find(stat => stat.name === 'shotsOnTarget');
